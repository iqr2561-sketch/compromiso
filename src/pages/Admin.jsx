...
import {
    ...
    ChevronLeft, ChevronRight, ChevronDown, Clock, Cpu, Sparkles, Wand2, View, Sun, Moon, MessageSquare, MessageCircle, Eye, EyeOff, History, GripVertical, CloudSun, Activity, Lock, User, Loader2
} from 'lucide-react';
...
const Admin = () => {
    ...
    const handleTestAiConfig = async () => {
        ...
        try {
            ...
            if (res.ok) {
                showToast("¡Conexión Exitosa! - IA 100% Operativa 🚀", "success");
            } else {
                const data = await res.json();
                showToast(`Error: ${data.error || 'Desconocido'}`, "error");
            }
        } catch (error) {
            showToast("Error de red al contactar la IA", "error");
        } finally {
                            ...
                            {pharmacyMode === 'list' ? (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-1 flex flex-col gap-6">
                                        <div className="bg-white dark:bg-[#11141b] p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm">
                                            <h4 className="text-[10px] font-black uppercase text-primary tracking-[0.2em] mb-4">Programador Automático</h4>
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-bold leading-relaxed mb-6">Selecciona una fecha y asigna el establecimiento que estará de turno ese día.</p>

                                            {/* Calendar Component */}
                                            <div className="bg-slate-50 dark:bg-[#0a0c10] border border-gray-200 dark:border-white/5 rounded-2xl p-4 mb-4">
                                                {/* Calendar Header */}
                                                <div className="flex items-center justify-between mb-4">
                                                    ...
                                                    <span className="text-xs font-black uppercase text-slate-900 dark:text-white tracking-widest">
                                                ...
                                                <div className="grid grid-cols-7 gap-1 mb-2">
                                                    {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((day, i) => (
                                                        <div key={i} className="text-center text-[9px] font-black text-slate-400 dark:text-slate-600 uppercase py-1">
                                                            {day}
                                                        </div>
                                                    ))}
                                                </div>
                                                            ...
                                                            days.push(
                                                                <button
                                                                    ...
                                                                    className={`aspect-square rounded-lg text-[10px] font-bold transition-all relative ${isSelected
                                                                        ...
                                                                        } ${isToday ? 'ring-2 ring-primary/20 dark:ring-primary/50' : ''} `}
                                                                >
                                                                    {day}
                                    ...
                                    <div className="lg:col-span-2">
                                        <div className="bg-white dark:bg-[#11141b] rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm">
                                            <div className="bg-slate-50 dark:bg-white/5 px-8 py-5 border-b border-gray-200 dark:border-white/5 flex items-center justify-between">
                                                <h4 className="text-[10px] font-black uppercase text-slate-700 dark:text-white tracking-[0.2em]">Calendario de Turnos</h4>
                                            ...
                                            <div className="p-2">
                                                {upcomingDuties.length > 0 ? (
                                                            ...
                                                            {upcomingDuties.map((duty, idx) => (
                                                                <tr key={idx} className="group hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                                                                    <td className="px-6 py-4">
                                                                        <div className="flex items-center gap-3">
                                                                            <CalendarIcon size={14} className="text-primary" />
                                                                            <span className="text-[11px] font-black text-slate-900 dark:text-white italic">{duty.date}</span>
                                                                        </div>
                                                                    </td>
                                                                    <td className="px-6 py-4">
                                                                        <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">{duty.pharmacy?.name}</span>
                                                                    </td>
                                                                    <td className="px-6 py-4 text-right">
                                                                        <button onClick={() => setDuty(duty.date, null)} className="p-2 text-slate-400 hover:text-red-500 transition-colors"><X size={14} /></button>
                                                                    </td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                                                    ...
                                                    <div className="py-20 text-center">
                                                        <Clock size={32} className="mx-auto text-slate-200 dark:text-slate-800 mb-4" />
                                                        <p className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-600 tracking-widest">No hay turnos programados a futuro</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                            ...
                                            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        >
                                                            <option value="gemini-2.5-flash" style={{ color: 'black' }}>Gemini 2.5 Flash (Última Tech - Confirmado)</option>
                                                            <option value="gemini-1.5-flash" style={{ color: 'black' }}>Gemini 1.5 Flash (Velocidad - Recomendado)</option>
                                                            <option value="gemini-1.5-pro" style={{ color: 'black' }}>Gemini 1.5 Pro (Mayor Calidad)</option>
                                                        </select>
                                                ...
                                                <div className="flex justify-end mt-2 md:col-span-2">
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-emerald-500/20 text-slate-300 hover:text-emerald-500 border border-white/10 hover:border-emerald-500/50 rounded-xl transition-all text-xs font-black uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed group/btn"
                                                    >
                                                        {isTestingAi ? (
                                                            <>
